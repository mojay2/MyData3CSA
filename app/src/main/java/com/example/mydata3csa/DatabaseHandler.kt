...
//            do{
//                userId = cursor.getInt(cursor.getColumnIndex("id")) //param may be KEY_ID as well
...
//    }
    ...
    fun viewEmployee(): List<EmpModelClass> {
        ...
        if (cursor.moveToFirst()) {
            do {
                val userIdIndex = cursor.getColumnIndex("id")
                val userNameIndex = cursor.getColumnIndex("name")
                val userEmailIndex = cursor.getColumnIndex("email")

                // Handle NULL values
                userId = if (!cursor.isNull(userIdIndex)) cursor.getInt(userIdIndex) else 0
